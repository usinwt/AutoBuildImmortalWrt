...
if [ "$count" -eq 1 ]; then
    ...
    if [ -f "$IP_VALUE_FILE" ]; then
        ...
        uci set network.lan.ipaddr='10.10.10.21'
        echo "default router ip is 10.10.10.21" >> $LOGFILE
    fi
...
sed -i "s/ r[^']*/'/" "$FILE_PATH"
sed -i "s/\(DISTRIB_DESCRIPTION='[^']*\)'/\1 Packaged by S.x'/" "$FILE_PATH"
#NEW_DESCRIPTION="Packaged by S.x"
